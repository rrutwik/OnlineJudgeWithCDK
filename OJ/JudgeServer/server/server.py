...
from service import healthcare
...
def downloadTestCase(test_case_id):
        print_to_stdout("Downloading file "+test_case_id+ " from S3Bucket "+ bucketname)
        with open(os.path.join(TEST_CASE_DIR,test_case_id+".zip"), 'wb') as f:
            s3_client.download_fileobj(bucketname, test_case_id+".zip", f)
        with zipfile.ZipFile(os.path.join(TEST_CASE_DIR,test_case_id+".zip"), 'r') as zip_ref:
            zip_ref.extractall(os.path.join(TEST_CASE_DIR, test_case_id))
        os.remove(os.path.join(TEST_CASE_DIR,test_case_id+".zip"))
...
class JudgeServer:
    ...
    def judge(cls, language_config, src, max_cpu_time, max_memory, test_case_id=None, test_case=None,
        ...
        # init
        if os.path.exists(os.path.join(TEST_CASE_DIR, test_case_id)) is False:
            downloadTestCase(test_case_id)
...
def server(path):
    if path in ("healthcheck"):
        healthCheck()
        ret = {"err": None, "data": "HealthCheck"}
    elif request.method not in ["POST"]:
        ret = {"err": "InvalidRequest", "data": "404"}
    elif path in ("judge", "ping", "compile_spj"):

    ...
    def __enter__(self) -> [JudgeServer, None]:
        with transaction.atomic():
            ...
            for server in servers:
                if temp is None:
                    temp = server
                elif server.task_number > temp.task_number:
                    temp = server
        if temp is not None:
            temp.task_number = F("task_number") + 1
            temp.save(update_fields=["task_number"])
            self.server = temp
        return temp
...
class SPJCompiler(DispatcherBase):
    ...
    def compile_spj(self):
        with ChooseJudgeServer() as server:
            ...
            logger.error(str(server.service_url))
...
class JudgeDispatcher(DispatcherBase):
    ...
    def judge(self):
        ...
        logger.error("Data sending => "+json.dumps(data))
        with ChooseJudgeServer() as server:
            if not server:
                data = {"submission_id": self.submission.id, "problem_id": self.problem.id}
                cache.lpush(CacheKey.waiting_queue, json.dumps(data))
                return
            logger.error("Server URL=> "+str(server.service_url))

...
class TestCaseZipProcessor(object):
    def uploadDirectory(self, path, name, spj, bucketname=bucketName):
        name_list = self.filter_name_list(os.listdir(path), spj)
        name_list.append("info")
        file_name = os.path.join(path, name + ".zip")
    ...
    def process_zip(self, uploaded_zip_file, spj, dir=""):
        ...
        self.uploadDirectory(test_case_dir, test_case_id, spj)
...
#               s3_client.download_fileobj(bucketname, test_case_id+".zip", f)
    ...
    def get(self, request):
        ...
        file_name = self.downloadTestCase(problem.test_case_id)
        ...
        os.remove(file_name)
        shutil.rmtree(os.path.join(settings.TEST_CASE_DIR, problem.test_case_id))
        return response
...
class ExportProblemAPI(APIView):
    ...
    def get(self, request):
        return self.error("Function Not Available")
